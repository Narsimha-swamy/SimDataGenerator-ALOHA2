...
def _get_qpos(physics: dm_control.mujoco.Physics,
              target_mocap_pose:dict):
    
    '''
    function to get IK solution in joint position space for given target pose in EE space

    args: 
        physics: physics class containing all model information
        target_mocap_pose: 
                    left:
                        pos: xyz
                        quat: 
                    ...
                    right:
                        ...
                        gripper:
    return: 
        qpos: IK action 14 dim = 2 * (6 joints + 1 gripper)
    '''
    ...
    # proicess action from input set mocap to target location 
    left_action,right_action = np.split(target_mocap_pose,2)
    left_pos = left_action[:3]
    left_quat = left_action[3:7]
    left_grip = left_action[-1]
    right_pos = right_action[:3]
    right_quat = right_action[3:7]
    right_grip = right_action[-1]
    

    data.mocap_pos[l_mid] = left_pos
    data.mocap_quat[l_mid] = left_quat

    data.mocap_pos[r_mid] = right_pos
    data.mocap_quat[r_mid] = right_quat
    ...
    qpos =  configuration.q[dof_ids].copy()
    left_qpos,right_qpos = np.split(qpos,2)

    rate.sleep()

    return np.concatenate((left_qpos,[left_grip],
                           right_qpos,[right_grip])) # Ik_actions
